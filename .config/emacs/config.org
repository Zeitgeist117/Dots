...
(global-visual-line-mode)  ;; Enable truncated lines
...
(use-package general
	...
	(zg/leader-keys
	 ...
	 (zg/leader-keys
	   ...
	   "m f" '(org-toggle-item :wk "Org toggle item")
	   ...
	   "m i" '(org-roam-node-insert-immediate :wk "Org Roam insert node immediately")
...
(defun my/set-fonts ()
  ...
  (cond
   ((find-font (font-spec :name "CozetteVector"))
    (set-face-attribute 'default nil :font "CozetteVector" :height 140 :weight 'medium)
    (set-face-attribute 'bold nil :weight 'extra-bold))
   ((find-font (font-spec :name "CozetteVector"))
    (set-face-attribute 'default nil :font "CozetteVector" :height 140)))
  (custom-set-faces
...
(use-package ox-typst
  :straight t
  :after org)
(setq org-typst-link-roam-as-text t)
#+end_src
...
(custom-theme-set-faces
 'user
 ;; Pin fixed-pitch so it doesn't bleed into your default font
...
*** Org-Roam Media Organisation
#+begin_src emacs-lisp 
(with-eval-after-load 'org-roam

  (defvar my/org-roam-media-types
    '(("book"      . (("Title"    . "id-link")
                      ("Genre"    . "GENRE")
                      ("Author"   . "AUTHOR")
					  ("Series" . "SERIES")
...
(defvar my/org-roam-media-table-files
  '(("book"      . "~/Notes/roam/20260617081526-books.org")
    ("anime"     . "~/Notes/roam/20260206174751-anime.org")
    ("tv"        . "~/Notes/roam/20260206173132-tv.org")
    ("game" . "~/Notes/roam/20260206172943-games.org")
    ("movie"     . "~/Notes/roam/20260206173211-movies.org")))
  (defun org-dblock-write:media-table (params)

    (let* ((type    (plist-get params :type))
           (columns (alist-get type my/org-roam-media-types nil nil #'string=))
           (nodes   (org-roam-db-query [:select [id title properties]
                                        :from nodes])))
      (when (and type columns)
        (insert (concat "| " (mapconcat #'car columns " | ") " |\n"))
        (insert (concat "|-" (mapconcat (lambda (_) "-") columns "-+-") "-|\n"))
        (dolist (node nodes)
          (let* ((id    (nth 0 node))
                 (title (nth 1 node))
                 (props (nth 2 node)))
            (when (equal (alist-get "TYPE" props nil nil #'string=) type)
              (insert (concat "| "
                              (mapconcat
                               (lambda (col)
                                 (if (equal (cdr col) "id-link")
                                     (format "[[id:%s][%s]]" id title)
                                   (or (alist-get (cdr col) props nil nil #'string=) "")))
                               columns
                               " | ")
                              " |\n"))))))))

(defun my/org-roam-refresh-media-tables ()
  (when (org-roam-file-p)
    (let* ((type      (org-entry-get (point-min) "TYPE"))
           (table-file (alist-get type my/org-roam-media-table-files nil nil #'string=)))
      (when table-file
        (with-current-buffer (find-file-noselect table-file)
          (org-update-all-dblocks)
          (save-buffer))))))

  (add-hook 'after-save-hook #'my/org-roam-refresh-media-tables))

#+end_src

#+RESULTS:
| my/org-roam-refresh-media-tables | doom-modeline-update-vcs | doom-modeline-update-buffer-file-name |
...
#+begin_src emacs-lisp
  ...
  (use-package qml-mode :straight t)
#+end_src
** Diminish
#+begin_src emacs-lisp
